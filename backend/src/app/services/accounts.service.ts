...
export class AccountService {
  ...
  createRegister = async ({ username, email, password, iv, picture }, accountRepository) => {
    try {
  ...
  login = async ({ email, password, iv }) => { 
    ...
    if (!account) {
      throw new GraphQLError('El correo electrónico o la contraseña son incorrectos')
    }
  ...
  register = async ({ username, email, password, iv }) => {
    ...
    if (exitsAccount) throw new GraphQLError('El correo electrónico se encuentra registrado.')
  ...
  updateAccount = async (args) => {
    const { username, status, password, iv, accountId } = args
