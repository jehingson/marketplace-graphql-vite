...
let PublicService = class PublicService {
    constructor() {
        this.products = ({ limit, offset, inputValue, range }) => __awaiter(this, void 0, void 0, function* () {
            console.log("prices", range);
            try {
                const resposity = data_source_1.AppDataSource.getRepository(products_model_1.Products);
                const result = yield resposity
                    .createQueryBuilder("products")
                    .leftJoinAndSelect("products.account", "account")
                    .where(`(products.name LIKE :name OR products.sku LIKE :sku) AND products.prices BETWEEN :minPrice AND :maxPrice`, {
                    name: `%${inputValue}%`,
                    sku: `%${inputValue}%`,
                    minPrice: `${range[0]}`,
                    maxPrice: `${range[1]}`
